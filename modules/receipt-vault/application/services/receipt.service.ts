...
export class ReceiptService {
  ...
  async getReceipt(receiptId: string, workspaceId: string): Promise<Receipt | null> {
  ...
  async getReceiptsByExpense(expenseId: string, workspaceId: string): Promise<Receipt[]> {
    return await this.receiptRepository.findByExpenseId(expenseId, workspaceId)
  }

  async getReceiptsByUser(userId: string, workspaceId: string): Promise<Receipt[]> {
  ...
  async filterReceiptsPaginated(filters: ReceiptFilters & { page?: number; pageSize?: number }): Promise<{
    data: Receipt[]
    pagination: {
      page: number
      pageSize: number
  ...
  async linkToExpense(receiptId: string, expenseId: string, workspaceId: string): Promise<Receipt> {
  ...
  async unlinkFromExpense(receiptId: string, workspaceId: string): Promise<Receipt> {
    const receipt = await this.receiptRepository.findById(
      ReceiptId.fromString(receiptId),
      workspaceId
    )
  ...
  async verifyReceipt(receiptId: string, workspaceId: string): Promise<Receipt> {
    const receipt = await this.receiptRepository.findById(
      ReceiptId.fromString(receiptId),
      workspaceId
    )
  ...
  async rejectReceipt(receiptId: string, workspaceId: string, reason?: string): Promise<Receipt> {
    const receipt = await this.receiptRepository.findById(
      ReceiptId.fromString(receiptId),
      workspaceId
    )
  ...
  async setThumbnail(receiptId: string, thumbnailPath: string, workspaceId: string): Promise<Receipt> {
    const receipt = await this.receiptRepository.findById(
      ReceiptId.fromString(receiptId),
      workspaceId
    )
  ...
  async deleteReceipt(receiptId: string, workspaceId: string, permanent: boolean = false): Promise<void> {
    const receipt = await this.receiptRepository.findById(
      ReceiptId.fromString(receiptId),
      workspaceId
    )
  ...
  async restoreReceipt(receiptId: string, workspaceId: string): Promise<Receipt> {
    const receipt = await this.receiptRepository.findById(
      ReceiptId.fromString(receiptId),
      workspaceId
    )
  ...
  async addMetadata(params: {
    ...
    const existing = await this.metadataRepository.findByReceiptId(receipt.getId())
    if (existing) {
  ...
  async updateMetadata(receiptId: string, workspaceId: string, updates: any): Promise<ReceiptMetadata> {
    const receipt = await this.receiptRepository.findById(
      ReceiptId.fromString(receiptId),
      workspaceId
    )
    ...
    // Apply updates
    if (updates.merchantName !== undefined || updates.merchantAddress !== undefined ||
        updates.merchantPhone !== undefined || updates.merchantTaxId !== undefined) {
      metadata.updateMerchantInfo(updates)
    }

    if (updates.subtotal !== undefined || updates.taxAmount !== undefined ||
        updates.tipAmount !== undefined || updates.totalAmount !== undefined ||
        updates.currency !== undefined) {
      metadata.updateFinancialAmounts(updates)
    }

    if (updates.transactionDate !== undefined || updates.transactionTime !== undefined) {
      metadata.updateTransactionInfo(updates.transactionDate, updates.transactionTime)
    }
  ...
  async getMetadata(receiptId: string, workspaceId: string): Promise<ReceiptMetadata | null> {
    const receipt = await this.receiptRepository.findById(
      ReceiptId.fromString(receiptId),
      workspaceId
    )
  ...
  // Tag management
  async addTag(receiptId: string, tagId: string, workspaceId: string): Promise<void> {
    const receipt = await this.receiptRepository.findById(
      ReceiptId.fromString(receiptId),
      workspaceId
    )
    ...
    const hasTag = await this.tagRepository.hasTag(receipt.getId(), TagId.fromString(tagId))
  ...
  async removeTag(receiptId: string, tagId: string, workspaceId: string): Promise<void> {
    const receipt = await this.receiptRepository.findById(
      ReceiptId.fromString(receiptId),
      workspaceId
    )
    ...
    await this.tagRepository.removeTag(receipt.getId(), TagId.fromString(tagId))
  }

  async getReceiptTags(receiptId: string, workspaceId: string): Promise<TagId[]> {
    const receipt = await this.receiptRepository.findById(
      ReceiptId.fromString(receiptId),
      workspaceId
    )
  ...
  async getReceiptStats(workspaceId: string): Promise<{
    ...
    const [total, pending, processing, processed, failed, verified] = await Promise.all([
      this.receiptRepository.countByWorkspace(workspaceId),
      this.receiptRepository.countByStatus(workspaceId, ReceiptStatus.PENDING),
      this.receiptRepository.countByStatus(workspaceId, ReceiptStatus.PROCESSING),
      this.receiptRepository.countByStatus(workspaceId, ReceiptStatus.PROCESSED),
      this.receiptRepository.countByStatus(workspaceId, ReceiptStatus.FAILED),
      this.receiptRepository.countByStatus(workspaceId, ReceiptStatus.VERIFIED),
    ])
