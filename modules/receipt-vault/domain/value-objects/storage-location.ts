...
export class StorageLocation {
  ...
  private validate(): void {
    if (!this.props.provider) {
      throw new Error('Storage provider cannot be empty')
    }
    ...
    if (this.props.provider !== StorageProvider.LOCAL) {
      if (!this.props.bucket || this.props.bucket.trim().length === 0) {
        throw new Error(`Bucket/container name is required for ${this.props.provider}`)
      }

      if (!this.props.key || this.props.key.trim().length === 0) {
        throw new Error(`Storage key/blob name is required for ${this.props.provider}`)
      }
    }
  }
