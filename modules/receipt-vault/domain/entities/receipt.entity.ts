...
import { MIN_OCR_CONFIDENCE, MAX_OCR_CONFIDENCE } from '../constants/receipt.constants'
...
export class Receipt {
  ...
  linkToExpense(expenseId: string): void {
    if (!expenseId || expenseId.trim().length === 0) {
      throw new ReceiptValidationError('expenseId', 'Expense ID cannot be empty')
    }

    if (this.isDeleted()) {
      throw new InvalidReceiptOperationError('link to expense', 'Receipt has been deleted')
    }
  ...
  markAsProcessed(ocrText?: string, ocrConfidence?: number): void {
    ...
    if (ocrConfidence !== undefined) {
      if (ocrConfidence < MIN_OCR_CONFIDENCE || ocrConfidence > MAX_OCR_CONFIDENCE) {
  ...
  markAsFailed(reason: string): void {
    if (!reason || reason.trim().length === 0) {
      throw new ReceiptValidationError('failureReason', 'Failure reason cannot be empty')
    }
  ...
  setThumbnailPath(path: string): void {
    if (!path || path.trim().length === 0) {
      throw new ReceiptValidationError('thumbnailPath', 'Thumbnail path cannot be empty')
    }
