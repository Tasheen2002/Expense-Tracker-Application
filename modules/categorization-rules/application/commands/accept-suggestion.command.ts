...
export class AcceptSuggestionHandler implements ICommandHandler<AcceptSuggestionCommand, CommandResult<CategorySuggestion>> {
  constructor(private readonly suggestionService: CategorySuggestionService) {}

  async handle(command: AcceptSuggestionCommand): Promise<CommandResult<CategorySuggestion>> {
    try {
      ...
      return CommandResult.failure<CategorySuggestion>('Failed to accept category suggestion')
    }
  }
}
