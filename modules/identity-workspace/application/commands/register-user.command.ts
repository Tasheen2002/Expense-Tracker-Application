...
export class RegisterUserHandler
  implements ICommandHandler<RegisterUserCommand, CommandResult<User>>
{
  ...
  async handle(command: RegisterUserCommand): Promise<CommandResult<User>> {
    try {
      ...
      if (!command.password || typeof command.password !== 'string') {
        return CommandResult.failure<User>('Password is required', ['password'])
      }

      if (command.password.length < 8) {
        return CommandResult.failure<User>('Password must be at least 8 characters', [
          'password',
        ])
      }
      ...
      if (error instanceof Error) {
        return CommandResult.failure<User>('User registration failed', [error.message])
      }
