...
export async function registerIdentityWorkspaceRoutes(
  ...
  const getUserHandler = new GetUserHandler(services.userManagementService);

  // Initialize command/query handlers - Workspace
  const createWorkspaceHandler = new CreateWorkspaceHandler(
    services.workspaceManagementService
  );
  const updateWorkspaceHandler = new UpdateWorkspaceHandler(
    services.workspaceManagementService
  );
  const deleteWorkspaceHandler = new DeleteWorkspaceHandler(
    services.workspaceManagementService
  );
  ...
  const acceptInvitationHandler = new AcceptInvitationHandler(
    services.workspaceInvitationService
  );
  const cancelInvitationHandler = new CancelInvitationHandler(
    services.workspaceInvitationService
  );
