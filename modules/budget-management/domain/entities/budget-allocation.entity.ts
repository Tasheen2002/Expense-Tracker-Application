...
export class BudgetAllocation {
  ...
  static create(data: CreateBudgetAllocationData): BudgetAllocation {
    ...
    if (allocatedAmount.decimalPlaces() > 2) {
      throw new Error('Allocated amount cannot have more than 2 decimal places')
    }
  ...
  updateSpentAmount(amount: number | string | Decimal): void {
    ...
    if (newAmount.decimalPlaces() > 2) {
      throw new Error('Spent amount cannot have more than 2 decimal places')
    }
  ...
  incrementSpent(amount: number | string | Decimal): void {
    const incrementAmount = typeof amount === 'number' || typeof amount === 'string'
      ? new Decimal(amount)
      : amount
  ...
  decrementSpent(amount: number | string | Decimal): void {
    const decrementAmount = typeof amount === 'number' || typeof amount === 'string'
      ? new Decimal(amount)
      : amount

    if (decrementAmount.isNegative() || decrementAmount.isZero()) {
      throw new Error('Decrement amount must be greater than zero')
    }

    const newSpent = this.props.spentAmount.sub(decrementAmount)
    if (newSpent.isNegative()) {
      throw new Error('Spent amount cannot become negative')
    }
  ...
  getSpentPercentage(): number {
    ...
    return this.props.spentAmount.div(this.props.allocatedAmount).mul(100).toNumber()
  }
  ...
  isFullySpent(): boolean {
    return this.props.spentAmount.greaterThanOrEqualTo(this.props.allocatedAmount)
  }
