...
export class SpendingLimit {
  ...
  static create(data: CreateSpendingLimitData): SpendingLimit {
    ...
    const limitAmount = typeof data.limitAmount === 'number' || typeof data.limitAmount === 'string'
    ...
    if (limitAmount.decimalPlaces() > 2) {
      throw new Error('Limit amount cannot have more than 2 decimal places')
    }
    ...
    if (!data.userId && !data.categoryId) {
      // Workspace-wide limit is valid (both null)
    }
  ...
  activate(): void {
    if (this.props.isActive) {
      throw new Error('Spending limit is already active')
    }
  ...
  deactivate(): void {
    if (!this.props.isActive) {
      throw new Error('Spending limit is already inactive')
    }
