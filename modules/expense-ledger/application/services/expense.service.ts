...
export class ExpenseService {
  ...
  async createExpense(params: {
    ...
    if (params.tagIds && params.tagIds.length > 0) {
      ...
      const tags = await this.tagRepository.findByIds(tagIdObjects, params.workspaceId)
  ...
  async updateExpense(
    ...
    if (!expense.canBeEdited()) {
      throw new Error('Expense cannot be edited in its current status')
    }
  ...
  async deleteExpense(expenseId: string, workspaceId: string, userId: string): Promise<void> {
    const expense = await this.expenseRepository.findById(
      ExpenseId.fromString(expenseId),
      workspaceId
    )
    ...
    if (!expense.canBeDeleted()) {
      throw new Error('Expense cannot be deleted in its current status')
    }

    await this.expenseRepository.delete(ExpenseId.fromString(expenseId), workspaceId)
  }

  async getExpenseById(expenseId: string, workspaceId: string): Promise<Expense | null> {
    return await this.expenseRepository.findById(ExpenseId.fromString(expenseId), workspaceId)
  }
  ...
  async getExpensesByUser(userId: string, workspaceId: string): Promise<Expense[]> {
    return await this.expenseRepository.findByUser(userId, workspaceId)
  }

  async getExpensesByCategory(categoryId: string, workspaceId: string): Promise<Expense[]> {
  ...
  async getExpensesByStatus(status: ExpenseStatus, workspaceId: string): Promise<Expense[]> {
  ...
  async submitExpense(expenseId: string, workspaceId: string, userId: string): Promise<Expense> {
    const expense = await this.expenseRepository.findById(
      ExpenseId.fromString(expenseId),
      workspaceId
    )
  ...
  async approveExpense(expenseId: string, workspaceId: string): Promise<Expense> {
    const expense = await this.expenseRepository.findById(
      ExpenseId.fromString(expenseId),
      workspaceId
    )
  ...
  async rejectExpense(expenseId: string, workspaceId: string): Promise<Expense> {
    const expense = await this.expenseRepository.findById(
      ExpenseId.fromString(expenseId),
      workspaceId
    )
  ...
  async revertExpenseToDraft(
    ...
    if (expense.userId !== userId) {
      throw new Error('Unauthorized to revert this expense')
    }
  ...
  async markExpenseAsReimbursed(expenseId: string, workspaceId: string): Promise<Expense> {
    const expense = await this.expenseRepository.findById(
      ExpenseId.fromString(expenseId),
      workspaceId
    )
  ...
  async addTagToExpense(
    ...
    const tagExists = await this.tagRepository.exists(TagId.fromString(tagId), workspaceId)
    if (!tagExists) {
  ...
  async getTotalExpenseByWorkspace(
    ...
    const total = await this.expenseRepository.getTotalByWorkspace(workspaceId, currency)
  ...
  async getTotalExpenseByUser(
    ...
    const total = await this.expenseRepository.getTotalByUser(userId, workspaceId, currency)
    return { total, currency }
  }
