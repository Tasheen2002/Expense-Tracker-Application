...
export class PrismaExpenseWorkflowRepository implements ExpenseWorkflowRepository {
  ...
  async findByWorkspace(workspaceId: string): Promise<ExpenseWorkflow[]> {
  ...
  async findPendingByApprover(approverId: string, workspaceId: string): Promise<ExpenseWorkflow[]> {
    const rows = await this.prisma.expenseWorkflow.findMany({
      where: {
        workspaceId,
  ...
  async findByUser(userId: string, workspaceId: string): Promise<ExpenseWorkflow[]> {
    const rows = await this.prisma.expenseWorkflow.findMany({
      where: {
        userId,
        workspaceId,
  ...
  private toDomain(row: any): ExpenseWorkflow {
    const steps = row.steps.map((stepRow: any) =>
      ApprovalStep.reconstitute({
        ...
        delegatedTo: stepRow.delegatedTo ? UserId.fromString(stepRow.delegatedTo) : undefined,
