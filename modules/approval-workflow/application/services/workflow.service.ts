...
export class WorkflowService {
  ...
  async initiateWorkflow(params: {
    ...
    const existing = await this.workflowRepository.findByExpenseId(params.expenseId)
    if (existing) {
  ...
  async getWorkflow(expenseId: string, workspaceId: string): Promise<ExpenseWorkflow> {
  ...
  async approveStep(params: {
    ...
    const workflow = await this.workflowRepository.findByExpenseId(params.expenseId)
    ...
    if (workflow.isCompleted()) {
      throw new WorkflowAlreadyCompletedError(params.expenseId, workflow.getStatus())
    }
    ...
    if (currentStep.getCurrentApproverId().getValue() !== params.approverId) {
      throw new UnauthorizedApproverError(params.approverId, currentStep.getId().getValue())
    }
  ...
  async delegateStep(params: {
    ...
    if (currentStep.getCurrentApproverId().getValue() !== params.fromUserId) {
      throw new UnauthorizedApproverError(params.fromUserId, currentStep.getId().getValue())
    }
  ...
  async cancelWorkflow(expenseId: string, workspaceId: string): Promise<ExpenseWorkflow> {
  ...
  async listPendingApprovals(approverId: string, workspaceId: string): Promise<ExpenseWorkflow[]> {
    return await this.workflowRepository.findPendingByApprover(approverId, workspaceId)
  }

  async listUserWorkflows(userId: string, workspaceId: string): Promise<ExpenseWorkflow[]> {
    return await this.workflowRepository.findByUser(userId, workspaceId)
  }
}
