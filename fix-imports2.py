...
def process_file(filepath):
    ...
    new_content = re.sub(
        ...
        r"import { WorkspaceId } from '\g<1>identity-workspace/domain/value-objects/workspace-id.vo';",
        new_content
    )
    
    new_content = re.sub(
        r"import\s*\{\s*UserId\s*\}\s*from\s*'((?:\.\./)+)identity-workspace';",
        r"import { UserId } from '\g<1>identity-workspace/domain/value-objects/user-id.vo';",
        new_content
    )
    ...
    new_content = re.sub(
        ...
        r"import { ExpenseId } from '\g<1>expense-ledger/domain/value-objects/expense-id.vo';",
        new_content
    )

    new_content = re.sub(
        r"import\s*\{\s*CategoryId\s*\}\s*from\s*'((?:\.\./)+)expense-ledger';",
        r"import { CategoryId } from '\g<1>expense-ledger/domain/value-objects/category-id.vo';",
        new_content
    )
