...
export class SpendingLimitAlreadyInactiveError extends BudgetManagementError {
  constructor(limitId: string) {
    super(
      `Spending limit ${limitId} is already inactive`,
      'SPENDING_LIMIT_ALREADY_INACTIVE',
      400
    );
  }
}
...
export class BudgetAllocationExceededError extends BudgetManagementError {
  constructor(_budgetId: string, budgetTotal: number, attemptedTotal: number) {
    super(
